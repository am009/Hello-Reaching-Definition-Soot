...
// 基于soot.dava.toolkits.base.AST.interProcedural.ConstantFieldValueFinder;改的
public class ClassFieldCrossReferenceAnalysis {
    ...
    public Pair<Stmt,SootMethod> getSigleDefine(FieldRef ref) {
        ...
        if (stmtList.size() > 1) {
            System.out.println("【警报】域的赋值点大于一个（但是我们只返回了一个）: " + stmtList.toString());
        }
    ...
    private void computeFieldToValuesAssignedList() {
        ...
        for (SootClass s : appClasses) {
            System.out.println("【交叉引用】 开始分析: " + s.getName());
